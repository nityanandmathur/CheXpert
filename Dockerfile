...
# and `pip install chexpert` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "chexpert", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n chexpert -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "chexpert", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "chexpert", "YOUR_CONSOLE_SCRIPT"]
